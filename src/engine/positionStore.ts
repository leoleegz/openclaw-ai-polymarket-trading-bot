...
// Re-export LivePosition from original import
export type LivePosition = ReturnType<typeof getOpenPositions>[0];
...
export function getPositionsDueToClose(closeAfterSeconds: number): ReturnType<typeof getOpenPositions> {
  const now = Date.now();
